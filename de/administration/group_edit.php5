<?
// Includes
$DOMAIN_NAME="NKAS";	// Name der Domain für Include Files
$libDir="/WWW/nebenkostenabrechnungssystem/phplib/";
require_once("../../phplib/classes/UserManager-1.0/UserManager.inc.php5");
$MIN_GROUP_BASETYPE_NEED=UM_GROUP_BASETYPE_AUSHILFE_ERWEITERT;
require_once("../../phplib/session.inc.php5");
$MENU_SHOW = true; // Menü und Username zeigen

$HM = 2;
$UM = 1;
include("page_top.inc.php5");

// GET-Varibale holen
if( $_GET["editElement"]!="" )$_POST["editElement"]=$_GET["editElement"];
if( isset($_POST["editElement"]) && !is_numeric($_POST["editElement"]) )unset($_POST["editElement"]);

// Objekt initialisieren
$userGroup = new UserGroup($db);

// Zugriffsberechtigung prüfen
if( isset($_POST["editElement"]) && $userGroup->Load((int)$_POST["editElement"], $db)===true ){
	if( $userGroup->GetBaseType()>$_SESSION["currentUser"]->GetGroupBasetype($db) ){
		$REDIRECT_TARGET_URL=$DOMAIN_HTTP_ROOT."de/administration/user.php5?UID=".$UID;
		include($SHARED_FILE_SYSTEM_ROOT."templates/redirect.php5");
		exit;
	}
}

$form=new OneColumnForm( $DOMAIN_HTTP_ROOT."de/administration/user.php5?UID=".$UID, new GroupFormData($_POST, $userGroup, $db) );
$form->PrintData(); 

include("page_bottom.inc.php5"); // Ende Content, Javascript Teil um Höhe zu ermitteln, </body></html>
?>
<?php
/**
 * Status "Protokoll durch FMS freigeben"
 * 
 * @access   	public
 ...
 */
class NkasProtokollFreigabeFMS extends NkasStatusFormDataEntry
{
	...
	public function InitElements($loadFromObject)
	{
		...
		if ($sendProtocolAsAttachment)
		{
			...
			ob_start();
			?>
				<strong>Vorschau E-Mail Anhang</strong><br>
				<input type="hidden" name="createDownloadFile" id="createDownloadFile" value="" />
				<input type="button" value="Widerspruch anzeigen" onClick="DownloadFile(<?=DOCUMENT_TYPE_ANHANG;?>);" class="formButton2"/>

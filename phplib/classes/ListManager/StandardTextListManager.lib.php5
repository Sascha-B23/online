<?php
/**
 * ListData-Implementierung für die Textvorlagen
 * 
 * @access   	public
 ...
 */
class StandardTextListManager extends ListData 
{	
	/**
	 * Datenbankobjekt
	 * @var DBManager
	 ...
	 * @var int
	 */
	protected $type = StandardText::ST_TYPE_TEMPLATE;
	
	/**
	 * UserManager-Objekt
	 * @var StandardTextManager
	 */
	 ...
	 */
	public function StandardTextListManager(DBManager $db, $type, $name)
	{
		// Kostenartenmanager initialisieren
		$this->db = $db;
		$this->type = $type;
		$this->manager = new StandardTextManager($this->db);
		// Options Array setzen	
		$this->options["icon"]="textmodule.png";
		$this->options["icontext"]= $name;
		// Header definieren
	...
	public function Search($searchString, $orderBy, $orderDirection="ASC", $numEntrysPerPage=20, $currentPage=0)
	{
		...
		$return=null;
		$objects=$this->manager->GetStandardText($searchString, $this->data["datahead"][(int)$orderBy]["sortby"], $orderDirection=="ASC" ? 0 : 1, $currentPage, $numEntrysPerPage, "", $this->type);
		for ($a=0; $a<count($objects); $a++)
		{
			$return[$a][0]=$objects[$a]->GetName();
			$return[$a][1]=Array();
			$return[$a][1]["editUrl"] = "standardtext_edit.php5?editElement=".$objects[$a]->GetPKey();
	...
	{
		return $this->manager->GetStandardTextCount($searchString, $this->type);
	}

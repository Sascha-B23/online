 ...
 */
class WiderspruchspunktListData extends ListData 
{
	 ...
	 */
	public function WiderspruchspunktListData($db, $widerspruch)
	{
		$this->db = $db;
	...
	public function Search($searchString, $orderBy, $orderDirection="ASC", $numEntrysPerPage=20, $currentPage=0)
	{
		...
		{
			$return[$a][0]=($objects[$a]->IsHidden() ? "<font color='#888888'>" : '').$objects[$a]->GetRank().($objects[$a]->IsHidden() ? "</font>" : '');
			$return[$a][1]=($objects[$a]->IsHidden() ? "<font color='#888888'>" : '').$objects[$a]->GetTitle().($objects[$a]->IsHidden() ? "</font>" : '');
			...
			$return[$a][5]="<font color='#555555'>".$currency." ".HelperLib::ConvertFloatToLocalizedString($objects[$a]->GetKuerzungbetrag($this->db, Kuerzungsbetrag::KUERZUNGSBETRAG_EINSTUFUNG_GRAU))."</font>";
			$return[$a][6]=Array();
			$return[$a][6]["editUrl"] = "javascript:EditPos(".$objects[$a]->GetPKey().");";
			if ($objects[$a]->IsDeletable($this->db))
			{
				$return[$a][6]["deleteUrl"] = "FM_FORM";
			}

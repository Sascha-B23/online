<?php
/**
 * Diese Klasse repräsentiert ein RueckweisungsBegruendung
 * 
 * @access   	public
 ...
 */
class RueckweisungsBegruendung extends DBEntry 
{
	/**
	 * Typen
	 */
	const RWB_TYPE_WIDERSPRUCH = 0;
	const RWB_TYPE_TERMINEMAIL = 1;
	
	/**
	 * Was wurde zurückgewiesen
	 * @var int
	 */
	protected $rwType = self::RWB_TYPE_WIDERSPRUCH;
	 ...
	 * @var string
	 */
	const TABLE_NAME = "rueckweisungsBegruendungen";
	
	/**
	 * Konstruktor
	 * @param DBManager $db
	 */
	public function RueckweisungsBegruendung(DBManager $db)
	{
		$dbConfig = new DBConfig();
		$dbConfig->tableName = self::TABLE_NAME;
		$dbConfig->rowName = Array("rwType", "nummer", "datum", "begruendung", "widerspruch", "user", "userRelease");
		$dbConfig->rowParam = Array("INT", "BIGINT", "BIGINT", "LONGTEXT", "BIGINT", "BIGINT", "BIGINT");
	...
	protected function BuildDBArray(&$db, &$rowName, &$rowData)
	{
		...
		if ($this->nummer==-1)
		{
			...
			$data=$db->SelectAssoc("SELECT count(pkey) as count FROM ".self::TABLE_NAME." WHERE rwType=".$this->rwType." AND widerspruch=".$this->widerspruch->GetPKey());

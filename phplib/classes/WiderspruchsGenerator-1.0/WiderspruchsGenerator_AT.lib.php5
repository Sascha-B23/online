<?php
/**
 * Widerspruchsgenerator (Österreich)
 * 
 * @access   	public
 ...
 */
class WiderspruchsGenerator_AT extends WiderspruchsGenerator
{
	/**
	 * Textbausteine (pkeys)
	 */
	const WG_WIDERSPRUCHSPUNKT_HEIZKOSTEN = 38;									// Heizkosten
	const WG_WIDERSPRUCHSPUNKT_ENTSORGUNG = 43;									// Entsorgung
	const WG_WIDERSPRUCHSPUNKT_REINIGUNG_UND_PFLEGE = 44;						// Reinigung und Pflege
	const WG_WIDERSPRUCHSPUNKT_ALLGEMEINSTROM = 40;								// Allgemeinstrom
	const WG_WIDERSPRUCHSPUNKT_WARTUNG_INSTANDHALTUNG_INSTANDSETZUNG = 36;		// Wartung, Instandhaltung und Instandsetzung
	const WG_WIDERSPRUCHSPUNKT_AUSZAHLUNG_GUTSCHRIFT = 53;						// Auszahlung Gutschrift
	const WG_WIDERSPRUCHSPUNKT_REDUZIERUNG_VORAUSZAHLUNG = 54;					// Reduzierung Vorauszahlung
	const WG_WIDERSPRUCHSPUNKT_OEFFENTLICHE_ABGABEN = 42;						// Öffentliche Abgaben
	const WG_WIDERSPRUCHSPUNKT_VERSICHERUNGEN = 41;								// Versicherung
	const WG_WIDERSPRUCHSPUNKT_VERWALTUNG_UND_MANAGEMENT = 37;					// Verwaltung und Management
	const WG_WIDERSPRUCHSPUNKT_UMLAGEFLAECHE = 48;								// Umlagefläche
	const WG_WIDERSPRUCHSPUNKT_BERECHNUNGSFEHLER = 52;							// Berechnungsfehler
	const WG_WIDERSPRUCHSPUNKT_VORAUSZAHLUNG = 51;								// Vorauszahlung
	const WG_WIDERSPRUCHSPUNKT_NICHT_VEREINBARTE_ABRECHNUNGSPOSITIONEN = 35;	// Nicht vereinbarte Abrechnungspositionen
	const WG_WIDERSPRUCHSPUNKT_LUEFTUNG_UND_KLIMATISIERUNG = 39;				// Lüftung und Klimatisierung
	const WG_WIDERSPRUCHSPUNKT_SONSTIGES = 45;									// Sonstige
	const WG_WIDERSPRUCHSPUNKT_KOSTENSTEIGERUNG = 46;							// Kostensteigerung
	const WG_WIDERSPRUCHSPUNKT_ZUSAMMENSETZUNG_GU = 47;							// Zusammensetzung Gesamteinheiten Umlageschlüssel
	const WG_WIDERSPRUCHSPUNKT_ZUSAMMENSETZUNG_GESAMTKOSTEN = 49;				// Zusammensetzung Gesamtkosten 
	const WG_WIDERSPRUCHSPUNKT_ABGRENZUNG = 50;									// Abgrenzungen 
	const WG_WIDERSPRUCHSPUNKT_BERECHNUNGSFEHLER_ABRECHNUNGSERGEBNIS = 30;		// Berechnungsfehler Abrechnungsergebnis
	const WG_WIDERSPRUCHSPUNKT_GESAMTKOSTENDECKELUNG = 32;						// Gesamtkostendeckelung
	
	/**
	 * Konstruktor
	 * @param DBManager $db
	 */
	public function WiderspruchsGenerator_AT(DBManager $db)
	{
	...
	protected function ErzeugeKostenartenBedingungen($rsKostenartID, &$list, $summeBetragKundeProJahr, $sollwertAbrechnungszeitraum)
	{
		...
		switch($rsKostenartID)
		{
			...
			case RSKostenartManager::RS_KOSTENART_OEFFENTLICHE_ABGABEN:
				...
				break;
			default:
				// Wenn Kostenart nicht überschrieben wird, die Standardparamter dafür übernehmen
				$bedingungen = parent::ErzeugeKostenartenBedingungen($rsKostenartID, $list, $summeBetragKundeProJahr, $sollwertAbrechnungszeitraum);
				break;
		}
		return $bedingungen;
	}
